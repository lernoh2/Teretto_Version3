...
contract Joker_V5 is IERC777Recipient, DSMath, ReentrancyGuard {
    ...
    function tokensReceived(address operator, address from, address to, uint256 amount, bytes calldata userData, bytes calldata operatorData) external override onlyValidToken {
        ...
        require(from != address(0), "From cannot be the null address");
         ...
         // Owner adding liquidity
        if (from == owner) {
            handleOwner(amount);
            return;
        }

        // Regular user transfer: only trigger for exact amount
        if (amount == 7.5 * 10 ** 18) {
...
function handleTransfer(address from, uint256 amount) internal {
    ...
    // Always sync reserve1 with actual token balance
    reserve1 = tokenBalance();

    // Update counts and emit
    transfersCount++;
    emit TransfersCount(transfersCount.toString());

    // Recompute random numbers
    randomNumber1 = theRandomNumber1();
    randomNumber2 = theRandomNumber2();
    randomNumber3 = theRandomNumber3();
    ...
    // Trigger redistribution if sum == 21
    if (sum == 21) {
        transferredProfit = redistribute(from);
        triggered = true;
    }
...
function redistribute(address recipient) internal returns (uint256) {
    lastSender = recipient;

    // Compute profit based on actual token balance
    uint256 currentBalance = tokenBalance();
    uint256 profitToSend = DSMath.wdiv(DSMath.wmul(currentBalance, 30), 100);

    // Transfer and check success
    bool success = ETTO.transfer(recipient, profitToSend);
    require(success, "Redistribution transfer failed");

    // Update reserve1 to match actual balance after transfer
    reserve1 = tokenBalance();

    emit Redistributed(address(this), recipient, profitToSend);
...
function handleOwner(uint256 amount) internal {
    // Always sync reserve1 with real balance
    reserve1 = tokenBalance();
